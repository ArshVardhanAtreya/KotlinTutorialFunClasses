...
fun main() {
    ...
    val people =
        listOf(                                                     // Defines a collection of people.
    ...
    val phoneBook =
        people.associateBy { it.phone } // Builds a map from phone numbers to their owners' information. 'it.phone' is the keySelector here. The valueSelector is not provided, so the values of the map are Person objects themselves.
    val cityBook = people.associateBy(
        Person::phone,
        Person::city
    )  // Builds a map from phone numbers to the cities where owners live. Person::city is the valueSelector here, so the values of the map contain only cities.
    val peopleCities = people.groupBy(
        Person::city,
        Person::name
